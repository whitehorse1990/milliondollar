...
*/
        ...
        $sql_tooltip = "select * from ads where order_id='".$row['order_id']."'";
        $res_tooltip = mysql_query($sql_tooltip) or die (mysql_error().$sql_tooltip);
        $tooltip_row = mysql_fetch_array($res_tooltip);

        $ads_text = $tooltip_row['5'];
        $com_name = $tooltip_row['8'];
	...
	<?php if (ENABLE_MOUSEOVER=='NO') { ?>
		...
		alt="<?php echo htmlspecialchars($ads_text.$com_name);?>"
	<?php } ?>
	target="_blank" >
