...
// Contributed by Martin 
	...
	function render_map_area($fh,$data, $b_row) {
		...
		if (ENABLE_MOUSEOVER=='NO') {
		  ...
		  fwrite($fh, " title=\"".htmlspecialchars($data['alt_text'])."\" data=\"".htmlspecialchars($data['4'])."\" alt=\"".htmlspecialchars($data['alt_text'])."\"");
		}
		fwrite($fh, ">\n");
...
function process_map($BID, $map_file='') {
  ...
  $sql = "SELECT DISTINCT blocks.order_id, blocks.user_id,url,image_data,block_id,alt_text,MIN(x) AS x1,MAX(x) AS x2,MIN(y) AS y1,MAX(y) AS y2, blocks.ad_id, COUNT(*) AS Total, ads.*
                     FROM blocks, ads
                    WHERE (ads.ad_id = blocks.ad_id)
                      AND (published = 'Y')
					  AND (status = 'sold' )
                      AND (blocks.banner_id = '$BID')
                      AND (image_data > '')
                      AND (image_data = image_data)
                 GROUP BY blocks.order_id";
  $result = mysql_query ($sql) or die (mysql_error());
  ...
  while ($row = mysql_fetch_array($result)) {
	...
	if ( ( ($x_span * $y_span) / ($b_row['block_width']*$b_row['block_height']) ) != $row['Total'] ) {
	  ...
	  $sql_i = "SELECT DISTINCT url, image_data, block_id, alt_text, MIN(x) AS x1, MAX(x) AS x2, y AS y1, y AS y2, blocks.ad_id, COUNT(*) AS Total, ads.*
						   FROM blocks,ads
						  WHERE (ads.ad_id = blocks.ad_id)
						    AND (published = 'Y')
							AND (status = 'sold' )
							...
							AND (image_data > '')
							AND (image_data = image_data)
							AND (blocks.order_id = ".$row['order_id'].")
					   GROUP BY y";
	  ...
	  while ($row_i = mysql_fetch_array($res_i)) {
		...
		if ($box_count != $row_i['Total']) {
		  ...
		  $sql_r = "SELECT blocks.ad_id, url, image_data, block_id, alt_text, x AS x1, x AS x2, y AS y1, y AS y2, ads.*
					  FROM blocks, ads
					 WHERE (ads.ad_id = blocks.ad_id)
                       AND (published = 'Y')
					   AND (status = 'sold' )
